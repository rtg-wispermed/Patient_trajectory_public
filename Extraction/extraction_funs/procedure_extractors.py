...
def getRadioTherapy():
    ...
    ex.addCondition(lambda x: tryPath(x, ['identifier',0,'system']) == 'https://hospital.org/TumorDocumentation/RadiationTherapy/nNGM')
...
def getOperation():
    ...
    ex.addCondition(lambda x: tryPath(x, ['identifier',0,'system']) == 'https://hospital.org/TumorDocumentation/Operation/nNGM')
    ...
    ex.addField('cat_resection_type', lambda x: tryPath(x, 
                                                        ...
                                                        lambda x: tryPath(x, ['url']) == 'resection type'),
                False)
    ex.addField('cat_resection_status', getResectionStatus, False)
    ex.addField('cat_ops_code', lambda x: tryPath(x, ['code','coding','*','code'],
                                                 lambda x: tryPath(x, ['system']) == 'https://hospital.org/TumorDocumentation/Operation/OPS'), False)
    
    return ex

def getExaminations():
    ex = extractor(name = "Operation", resourceType = "procedure")
    ex.addCondition(lambda x: tryPath(x, ['identifier',0,'system']) == 'https://hospital.org/TumorDocumentation/ExaminationPerformed')
